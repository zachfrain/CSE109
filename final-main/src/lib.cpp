...
vec pack109::serialize(struct File item){ // [source][0]
    vec bytes;
...
struct File pack109::deserialize_file(vec bytes){ // [source][0]
   if(bytes.size() < 10)
        throw;
...
vec pack109::serialize(struct Request item){ // [source][0]
    vec bytes;
    bytes.push_back(PACK109_M8);
    bytes.push_back(0x1);
...
struct Request pack109::deserialize_request(vec bytes){ // [source][0]
...
// [0]: https://app.slack.com/client/T02V0GURGER/C02V3EGFYEN
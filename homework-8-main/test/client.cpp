...
int main(int argc, char *argv[])
{
    ...
    if (send_req == 2) {
        ...
        printf("Client: File has been serialized.\n");

        transferred = encrypt(serialized);
        printf("Client: File has been encrpyted.\n");
    } else if(send_req == 3){
        req.name = argv[4];

        serialized = pack109::serialize(req);
        printf("Client: Request has been serialized.\n");

        transferred = encrypt(serialized);
        printf("Client: Request has been encrypted.\n");
    }
    ...
    if (server == NULL)
    {
        ...
        printf("Client: Connected to host successfully.\n");
    }
    ...
    */
    ...
    if(n < 0){
        ...
        printf("Client: Success reading from socket.\n");
    }  
    ...
    if(n < 0){
        ...
        printf("Client: Success writing to socket.\n");
    }
...
./build/bin/release/client --hostname dyn231045.cc.lehigh.edu --send document.txt
